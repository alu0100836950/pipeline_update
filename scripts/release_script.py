...
class FileModifier:
    ...
    def adjust_latest_wc_version(self, version):
        ...
        # Si solo hay un número, agregamos un .0
        if len(parts) == 1:
...
class TaskManager:
    ...
    def procesar_archivo_rutas(self, gitpath, task, type_plugin, type_version, versions):
        ...
        if os.path.exists(gitpath):  # Validamos que sea una ruta existente
            plugin_name = os.path.basename(gitpath)  # Extraemos el nombre del plugin
            ...
            if response == 's':
                ...
                print(f"Ejecutando '{task}' en: {gitpath}")
                self.tasks[task](gitpath)  # Ejecuta la tarea sobre la ruta válida
            else:
                print(f"Omitiendo {plugin_name}")
        else:
            print(f"Ruta no válida: {gitpath}")
    ...
    def run_update(self, repo_path):
        try:
            ...
            if self.versions == {}:
                ...
                return
            print(f"Versiones a actualizar: {self.versions}")
...
if __name__ == "__main__":
    print("Ejecutando el script de release...")
    ...
    parser.add_argument("--gitpath", help="Ruta al repositorio Git", default=None)
    
    parser.add_argument("--type_version", help="Tipo de versión a actualizar", default=None)
    
    parser.add_argument("--versions", help="Versiones a actualizar")
    ...
    manager.procesar_archivo_rutas(args.gitpath, "update", args.type_plugins, args.type_version, versions)
